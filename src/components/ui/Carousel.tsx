...
/**
 * react-multi-carousel sometimes throws during SSR.
 * Use dynamic import with ssr: false so the carousel runs only on client side.
 */
...
export default function CustomCarousel() {
  const images = [
   ...
   return (
    <div className="max-w-7xl mx-auto mt-6">
      {/* Carousel is client-only via dynamic import */}
      {/* react-multi-carousel props */}
      <Carousel
        responsive={responsive}
        infinite
        autoPlay
      ...
      >
        {images.map((src, index) => (
          <div key={index} className="relative w-full h-[400px]">
            {/* Ensure Next Image has a positioned parent */}
